//! Provides constructs for the Raven app version and release channel.
...
pub static RELEASE_CHANNEL_NAME: LazyLock<String> = LazyLock::new(|| {
    if cfg!(debug_assertions) {
        env::var("RAVEN_RELEASE_CHANNEL")
            .unwrap_or_else(|_| include_str!("../RELEASE_CHANNEL").trim().to_string())
    } else {
...
pub fn app_identifier() -> &'static str {
    match *RELEASE_CHANNEL {
        ReleaseChannel::Dev => "Raven-Editor-Dev",
        ReleaseChannel::Nightly => "Raven-Editor-Nightly",
        ReleaseChannel::Preview => "Raven-Editor-Preview",
        ReleaseChannel::Stable => "Raven-Editor-Stable",
    }
}

/// The Git commit SHA that Raven was built at.
...
impl AppVersion {
    ...
    pub fn load(
        ...
        let mut version: Version = if let Ok(from_env) = env::var("RAVEN_APP_VERSION") {
            from_env.parse().expect("invalid RAVEN_APP_VERSION")
        } else {
...
/// A Raven release channel.
#[derive(Debug, Copy, Clone, PartialEq, Eq, Default)]
pub enum ReleaseChannel {
    /// The development release channel.
    ///
    /// Used for local debug builds of Raven.
    #[default]
    Dev,
...
impl ReleaseChannel {
    ...
    pub fn display_name(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "Raven Dev",
            ReleaseChannel::Nightly => "Raven Nightly",
            ReleaseChannel::Preview => "Raven Preview",
            ReleaseChannel::Stable => "Raven",
        }
    }
    ...
    /// and WM_CLASS on X11.
    /// This also has to match the bundle identifier for Raven on macOS.
    pub fn app_id(&self) -> &'static str {
        match self {
            ReleaseChannel::Dev => "dev.raven.Raven-Dev",
            ReleaseChannel::Nightly => "dev.raven.Raven-Nightly",
            ReleaseChannel::Preview => "dev.raven.Raven-Preview",
            ReleaseChannel::Stable => "dev.raven.Raven",
        }
    }
