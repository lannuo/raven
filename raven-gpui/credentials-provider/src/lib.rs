...
///
/// By default, running Raven in development uses the development credentials
...
static RAVEN_DEVELOPMENT_USE_KEYCHAIN: LazyLock<bool> = LazyLock::new(|| {
    std::env::var("RAVEN_DEVELOPMENT_USE_KEYCHAIN").is_ok_and(|value| !value.is_empty())
});
    ...
    fn new(cx: &App) -> Arc<Self> {
        let use_development_provider = match ReleaseChannel::try_global(cx) {
            Some(ReleaseChannel::Dev) => {
                ...
                //
                // However, if the `RAVEN_DEVELOPMENT_USE_KEYCHAIN` environment
                // variable is set, we will use the actual keychain.
                !*RAVEN_DEVELOPMENT_USE_KEYCHAIN
            }
...
/// re-allow access to the system keychain when developing Raven.
