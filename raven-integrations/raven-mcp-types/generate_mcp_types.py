...
def define_struct(
    ...
    for prop_name, prop in properties.items():
        ...
        if prop_name == "type" and type_default_fn:
            rs_prop.serde = append_serde_attr(
                rs_prop.serde, f'default = "{type_default_fn}"'
            )

        if prop_type.startswith("&'static str"):
            fields.append(
                StructField("const", rs_prop.name, prop_type, rs_prop.serde, rs_prop.ts)
            )
        else:
            fields.append(
                StructField("pub", rs_prop.name, prop_type, rs_prop.serde, rs_prop.ts)
            )

    # Special-case: add Raven-specific user_agent to Implementation
    if name == "Implementation":
        fields.append(
            StructField(
                ...
                "#[ts(optional)]",
                "This is an extra field that the Raven MCP server sends as part of InitializeResult.",
            )
        )
