...
use crate::AsyncBody;
use crate::{Builder, HttpRequest, Method, Response};
...
type FakeHttpHandler = Arc<
    dyn Fn(HttpRequest<AsyncBody>) -> BoxFuture<'static, anyhow::Result<Response<AsyncBody>>>
        + Send
        + Sync
        + 'static,
>;
...
impl FakeHttpClient {
    ...
    where
        ...
        F: Fn(HttpRequest<AsyncBody>) -> Fut + Send + Sync + 'static,
    {
    ...
    where
        Fut: futures::Future<Output = anyhow::Result<Response<AsyncBody>>> + Send + 'static,
        F: Fn(FakeHttpHandler, HttpRequest<AsyncBody>) -> Fut + Send + Sync + 'static,
    {
